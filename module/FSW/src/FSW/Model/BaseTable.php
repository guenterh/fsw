...
abstract class BaseTable
{
	...
	public function getSearchFieldsLikeCondition($searchString)
	{
        ...
        //$platform->setDriver($this->tableGateway->adapter->getDriver());


		foreach ($searchWords as $searchWord) {
			$likeWhere = new Where(null, Where::COMBINED_BY_OR);

            //$test = $platform->quoteValue($searchWord);

			foreach ($this->searchFields as $searchField) {
				$likeWhere->like($searchField,  '%' . $searchWord . '%' );
                //$tt = $platform->quoteTrustedValue( $searchField);
                //$likeWhere->like($platform->quoteIdentifier( $searchField), '%' .$searchWord . '%');
			}
...
//		var_dump($sql->getSqlStringForSqlObject($select));
		...
		$t = $this->tableGateway->selectWith($select);
        //$adapter = $this->tableGateway->getAdapter();
        //$test = $adapter->query($sql,Adapter::QUERY_MODE_EXECUTE );
        return $t;
	}
